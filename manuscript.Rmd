...
A study with 6,233 people from Germany found that "[f]requency, duration and diversity of media exposure were positively associated with more symptoms of depression" [@bendauAssociationsCOVID19Related2021, p. 283], which implies negative consequences for well-being.
...
First, the device (e.g., smartphone); second, the type of application (e.g., social networking site); third, the branded application (e.g., Twitter); fourth, the feature (e.g., status post); fifth, the interaction (e.g., one-to-many); sixth, the message (e.g., content) [@meierComputermediatedCommunicationSocial2020a].
...
The question of whether and how _general_ social media use affect well-being, on the other hand, is well-researched.
...
These overviews are well aligned with several individual new studies employing advanced methods [@orbenSocialMediaEnduring2019; @przybylskiDoesTakingShort2021a; @schemerImpactInternetSocial2021; @kerestesAdolescentsOnlineSocial2020].
For example, @beyensSocialMediaUse2021 reported that although for some users (roughly one quarter) the effects of social media use on well-being were negative, for almost the same number of users they were positive, while for the majority the effects were neutral.
...
Together, the strongest argument seems to be that _in general_ the effects of social media on well-being are, on average, small at best.
Because this study only looks at _one part_ of social media use---namely, COVID-19 related interactions---it is very focused, and the overall potential of the effects should diminish further.
...
Thus, only if we can control for _all_ confounders, can we correctly estimate causality without bias [@rohrerThinkingClearlyCorrelations2018].
...
Complicating matters, often it is unclear if a variable is a mediator or a confounder.[^collider]
...
In a statistical regression, _b_ estimates the change in the dependent variable if the independent variable increases by one point.
...
To illustrate, if the 95% confidence interval falls completely within the null-region (e.g., _b_ = .20, [95% CI: .15, .25]), the hypothesis that the effect is only trivial is supported.
...
For more information on the analyses, for example a complete documentation of the models and their results, see [companion website](https://tdienlin.github.io/Austrian_Corona_Panel/index.html).
...
Reliability was also very high, $\omega$ = `r rel_aff_pos %>% my_round("std")`.
...
Reliability was also high, $\omega$ = `r rel_aff_neg %>% my_round("std")`.
...
Respondents who used Instagram more frequently than usual to attain COVID-19 related news reported slightly _higher_ simultaneous levels of life satisfaction than usual (_b_ = `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Instagram" & type == "within") %>% select(estimate) %>% my_round("b")` [95% CI `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Instagram" & type == "within") %>% select(conf.low) %>% my_round("b")`, `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Instagram" & type == "within") %>% select(conf.high) %>% my_round("b")`]).
Respondents who used Twitter more frequently than usual to attain COVID-19 related news reported slightly _lower_ simultaneous levels of life satisfaction than usual (_b_ = `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Twitter" & type == "within") %>% select(estimate) %>% round(2)` [95% CI `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Twitter" & type == "within") %>% select(conf.low) %>% round(2)`, `r dat_fig_results_channel %>% filter(dv == "Life satisfaction" & iv == "Twitter" & type == "within") %>% select(conf.high) %>% my_round("b")`]).
...
In addition, as a rough estimate for the SESOI, we can use on the typical convention that small effects start at _r_ = |.10|.
...
Is it realistic to expect that extremely changing only _one_ of these aspects should already manifest in a detectable change in well-being?
Or would it make more sense to expect that thoroughly committing to _two_ activities (e.g. regularly exercising _and_ establishing a reading habit) should then cause detectable improvements in well-being?
...
Practically, this would imply a SESOI half as large as I have defined here, that is _b_ = |.15| for well-being and _b_ = |.075| for affect.
